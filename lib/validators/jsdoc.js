...
class JSDocValidator {
  ...
  validateJSDocContent(jsDoc, type, context) {
    ...
    if (context.params && context.params.length > 0) {
      context.params.forEach(param => {
        if (!jsDoc.includes(`@param`) && !jsDoc.includes(`@param {`) && !jsDoc.includes(`@param ${param.name}`)) {
          errors.push(`Missing @param documentation for parameter "${param.name}" in ${context.name}`);
        }
      });
    }

    // Check for @returns tag if function has return statement
    if (context.hasReturn && !jsDoc.includes('@returns') && !jsDoc.includes('@return')) {
      errors.push(`Missing @returns documentation for ${context.name}`);
    }
  ...
  validateFile(filePath) {
    ...
    const patterns = [
      ...
      {
        regex: /^(?:export\s+)?(?:function\s+)?(\w+)\s*\([^)]*\):\s*JSX\.Element|^(?:export\s+)?function\s+(\w+).*\{[\s\S]*?return\s*\(/,
        type: 'component',
        extract: (match, lineNum) => ({
