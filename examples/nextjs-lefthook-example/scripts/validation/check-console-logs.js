...
const {
  ...
  checkNextJsConfig
} = require('../../lib/validators/console-logs.js')
...
function main() {
  ...
  if (nextConfig.hasNextJs && nextConfig.hasConsoleRemoval) {
    ...
    console.log(`\x1b[36mℹ️  Console logs will be automatically removed by Next.js (${nextConfig.configFile})\x1b[0m`)
    process.exit(0)
  }
  ...
  if (hasViolations) {
    ...
    console.log('\x1b[33m💡 Remove console.log statements or use proper logging alternatives\x1b[0m')
    ...
    console.log('  - Wrap in development-only conditions: if (process.env.NODE_ENV === "development")')
